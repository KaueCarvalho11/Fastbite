...
void minimizarHeap(Heap *heap, int indice) {
  ...
  if (esquerda < heap->tamanho &&
      heap->pedidos[esquerda]->id > heap->pedidos[maior]->id) {
    maior = esquerda;
  }

  if (direita < heap->tamanho &&
      heap->pedidos[direita]->id > heap->pedidos[maior]->id) {
    maior = direita;
  }
...
Pedido *criarPedido(const char *descricao) {
  ...
  novoPedido->descricao[sizeof(novoPedido->descricao) - 1] =
...
void fazerPedido(Heap *heap) {
  ...
  fflush(stdout); // Garante que a mensagem seja exibida antes de esperar a
                  // entrada do usuário
